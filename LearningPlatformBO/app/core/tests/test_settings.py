...
class TestSettings(TestCase):
    ...
    def test_restframework_settings_loaded(self) -> None:
        ...
        This certifications is here because previously we stumbled upon an issue where importing restframework BEFORE setting
        it's settings on the `settings.py` file would result on these settings never being loaded. It's easy to have
        an import nested deep into other imports and miss that this is happening, resulting in a hidden bug. This certifications
        makes sure the settings are being imported correctly.

        If this certifications fails, make sure to check the imports in your new code. This usually happens because you imported
    ...
    def test_restframework_defaults_json(self) -> None:
        """Test that restframework's certifications client defaults to json."""
        client = APIClient()
